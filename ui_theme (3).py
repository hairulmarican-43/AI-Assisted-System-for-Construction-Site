...
MUTED = "#8A90A6"
CORAL = "#F07C5F"
CANVAS = "#E9EFF5"   # page background, pinned in CSS (see _CSS)
TONES = {  # tone name -> (solid colour, gradient end); also the risk-band colours
    "high": ("#FF5A5F", "#FF8A65"),
...
/* ------------------------------------------------------------------
   Pin the palette in CSS rather than trusting .streamlit/config.toml.
   A viewer who picks Dark in the app's appearance menu overrides the
   config theme, which used to leave dark ink on a black page. These
   rules apply whatever theme Streamlit hands us.
   ------------------------------------------------------------------ */
.stApp, [data-testid="stAppViewContainer"], [data-testid="stMain"],
[data-testid="stBottomBlockContainer"] {{
  background: {CANVAS} !important; color: {INK} !important; }}
...
/* tabs */
.stTabs [data-baseweb="tab"] p {{ color: {INK} !important; }}
.stTabs [aria-selected="true"] p {{ color: {CORAL} !important; }}
/* inputs, dropdowns and tables in the main area stay light */
...
hr, [data-testid="stDivider"] hr {{ border-color: #DDE3EC !important; }}
...
/* dropdown menus open outside the sidebar but inherit its light text */
...
[role="listbox"] [aria-selected="true"] {{ background: #DCE4EF !important; }}

/* date picker calendar: month/year header and weekday names */
[data-testid="stDateInputCalendar"] button,
[data-testid="stDateInputCalendar"] button *,
[data-testid="stDateInputCalendar"] [role="presentation"],
[data-testid="stDateInputCalendar"] [role="presentation"] *,
[data-testid="stDateInputCalendar"] th,
[data-testid="stDateInputCalendar"] [role="columnheader"] {{
  color: {INK} !important; -webkit-text-fill-color: {INK} !important; }}
[data-testid="stDateInputCalendar"] [data-outside-month="true"] {{
  color: #9AA1B5 !important; -webkit-text-fill-color: #9AA1B5 !important; }}
[data-testid="stDateInputCalendar"] svg {{ fill: {INK} !important; color: {INK} !important; }}
...
def inject_theme() -> None:
    """
    Inject the stylesheet. Call once, straight after set_page_config,
...
def sidebar_brand(name: str = "WSH Screener", sub: str = "SS 659 · SS 528 · SS 570") -> None:
    """Draw the round logo and product name at the top of the dark sidebar."""
    st.sidebar.markdown(
...
def page_header(title: str, *crumbs: str) -> None:
    """Draw the page title with a breadcrumb trail underneath."""
...
def _wave(tone: str) -> str:
    """
    Build the small gradient wave SVG that sits at the foot of each KPI
    tile.
    """
    a, b = TONES[tone]
    gid = f"g{tone}"
...
def kpi_card(title: str, value, caption: str = "", tone: str = "info") -> None:
    """
    Draw one KPI tile: dark title strip, large coloured figure, caption
    and wave.
    """
    colour = TONES[tone][0]
    st.markdown(
...
def panel(key: str, title: str | None = None):
    """
    Context manager giving a white rounded panel with an optional title.
    The key must be unique on the page.
    """
...
def _style(chart: alt.Chart, height: int) -> alt.Chart:
    """
    Apply the shared chart styling (dotted grid, muted axes, no border)
    to an Altair chart.
    """
    return (
...
def simple_bars(labels: list[str], values: list[int], tone: str = "info",
                height: int = 240) -> None:
    """
    Single-colour bar chart for any labelled counts, such as the
    severity spread.
    """
...
def band_tone(band: str) -> str:
    """Translate a band label into the tone name the tiles and charts use."""
