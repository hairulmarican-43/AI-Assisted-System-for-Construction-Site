...
APP_TITLE = "AI-Assisted System for Construction Site | Work-at-Height"
...
MODELS = {
    ...
    "Claude Opus 5 (high accuracy)": "claude-opus-5",
    "Claude Opus 4.8": "claude-opus-4-8",
    "Claude Fable 5 (most capable, highest cost)": "claude-fable-5",
}

# Rough per-image cost: ~5k input tokens (image + prompt), ~4k output tokens.
APPROX_COST_PER_FRAME_USD = {
    "claude-sonnet-5": 0.03,
    "claude-opus-5": 0.13,      # $5 / $25 per MTok
    "claude-opus-4-8": 0.13,    # $5 / $25 per MTok
    "claude-fable-5": 0.25,     # $10 / $50 per MTok
}
...
# Matched to the dashboard palette in ui_theme.py
RISK_COLOURS = {"Low": "#17A984", "Medium": "#E39A1B", "High": "#E8474C"}
...
def clause_display(ref: str) -> str:
    """
    Return a clause reference fit to show, or "" when it is still an
    unfilled placeholder. Keeps "TODO" out of screens and reports if a
    check is ever added before its clause is looked up.
    """
    ref = (ref or "").strip()
    if not ref or ref.upper().startswith("TODO"):
        return ""
    return ref


def unfilled_clauses(config: dict) -> int:
    """Count checks in checks.yaml whose clause_ref is still a placeholder."""
    return sum(
        1
        for cat in config["categories"].values()
        for chk in cat["checks"]
        if not clause_display(chk.get("clause_ref", ""))
    )
...
def call_claude(api_key: str, model: str, image: Image.Image,
    ...
    if stop_reason == "refusal":
        raise RuntimeError(
            f"{model} declined this request (safety classifier). "
            "Try again with Claude Sonnet 5 or Claude Opus 5."
        )
        ...
        for elem in rec.assessment.elements:
            ...
            for find in elem.findings:
                ...
                all_breaches.append({
                    ...
                    "clause": clause_display(find.clause_ref),
                })
    ...
    with info_col:
        ...
        if rpn == 0:
            ...
            st.markdown(
                ...
                f"<div style='font-size:.75rem;opacity:.85'>Highest risk</div>"
                f"<div style='font-size:1.5rem;font-weight:700'>{label} · "
                f"RPN {rpn}</div>"
                f"<div style='font-size:.85rem;margin-top:4px'>{action}</div>"
                f"</div>",
    ...
    if breaches:
        ...
        for elem, find in breaches:
            ...
            with st.expander(
                ...
                ref = clause_display(find.clause_ref)
                if ref:
        ...
        for elem in rec.assessment.elements:
            for find in elem.findings:
                ...
                writer.writerow([
                    ...
                    find.check_title, clause_display(find.clause_ref),
                    find.status,
...
def band_of(rpn: int, config: dict) -> str:
    return risk_band(rpn, config["risk_bands"])[0] if rpn else "None"
...
with st.sidebar:
    ui.sidebar_brand("WAH Screener", "SS 659 · SS 528 · SS 570")
    ...
    inspector = st.text_input("Name")
    position = st.text_input("Position")
    company = st.text_input("Company")
    project = st.text_input("Project")
    location = st.text_input("Project site")
    ...
    # Disappears once every check carries a reference; it does now.
    _pending = unfilled_clauses(config)
    if _pending:
        st.caption(
            f"{_pending} check(s) in `checks.yaml` have no clause reference "
            "yet. Fill them in from your own licensed copies of the "
            "standards."
        )

# --- Header ----------------------------------------------------------------

ui.page_header(APP_TITLE, "Dashboard", input_mode)
st.markdown(
    "<div class='wsh-intro'>Screens site photographs against <b>SS 659</b> "
    "(scaffolds), <b>SS 528</b> (personal fall-arrest systems) and "
    "<b>SS 570</b> (anchor devices and horizontal lifeline systems) in a "
    "single pass. Severity is reported on the WSH Risk Assessment scale."
    "</div>"
    "<div class='wsh-note'><b>Screening aid only.</b> This is not a formal "
    "inspection, does not discharge any duty under the WSH Act or its "
    "subsidiary legislation, and must not replace examination by a competent "
    "person or, for scaffolds, an approved Scaffold Supervisor. Assessment is "
    "limited to what is visible in the image — absence of a finding is not "
    "evidence of compliance.</div>",
    unsafe_allow_html=True,
)

# --- Session overview tiles (always visible) --------------------------------

_overview = compute_statistics(st.session_state.records, config)
_high = _overview["band_distribution"].get("High", 0)
_peak_band = band_of(_overview["peak_rpn"], config)

k1, k2, k3, k4 = st.columns(4)
with k1:
    ui.kpi_card("IMAGES SCREENED", _overview["image_count"],
                f"{_overview['element_count']} elements identified", "info")
with k2:
    ui.kpi_card("NON-COMPLIANCES", _overview["breach_count"],
                "across all three standards", "medium")
with k3:
    ui.kpi_card("HIGH RISK", _high, "need action before work continues",
                "high")
with k4:
    ui.kpi_card("PEAK RPN", _overview["peak_rpn"],
                f"band: {_peak_band}",
                ui.band_tone(_peak_band) if _overview["peak_rpn"] else "low")
...
with tab_screen:
    if input_mode == "Photograph":
        with ui.panel("upload", "Upload site photographs"):
            ...
            # Decode everything up front so unreadable files are reported
            # before any billable request is made.
            ...
            if loaded:
                ...
                st.caption(
                    ...
                    + (f" · {skipped} already screened this session and will "
                       "be skipped" if skipped else "")
                )
                ...
                with a_col:
                    rerun_all = st.checkbox(
                        "Re-screen images already in the register",
                        value=False)
                with b_col:
                    run_clicked = st.button(
                        "Run screening", type="primary", width="stretch",
        ...
        if run_clicked:
            if not api_key:
                ...
                if ok:
                    ...
                    worst_band = band_of(worst, config)
                    st.markdown("#### This batch")
                    m1, m2, m3, m4 = st.columns(4)
                    with m1:
                        ui.kpi_card("SCREENED", len(ok), "images", "info")
                    with m2:
                        ui.kpi_card("FAILED", len(errored), "images",
                                    "high" if errored else "low")
                    with m3:
                        ui.kpi_card("PEAK RPN", worst, "this batch",
                                    ui.band_tone(worst_band))
                    with m4:
                        ui.kpi_card("PEAK BAND", worst_band, "this batch",
                                    ui.band_tone(worst_band))
                    st.write("")
                ...
                if ok:
                    st.success(
                        ...
                        "Open the **Statistics & report** tab for collated "
                        "figures. The tiles at the top update on the next "
                        "interaction."
                    )
        ...
        if not CV2_AVAILABLE:
            ...
            with ui.panel("video", "Upload video walkthrough"):
                ...
                run_video = False
                if uploaded_video:
                    est = (APPROX_COST_PER_FRAME_USD.get(model, 0.03)
                           * max_frames)
                    st.info(
                    ...
                    run_video = st.button(
                        "Extract frames and run screening", type="primary")
                        ...
                        if not frames:
                            ...
                            for i, (frame_img, ts) in enumerate(frames):
                                try:
                                    ...
                                    if stop_reason == "max_tokens":
                                        a.truncated = True
                                    results.append((ts, frame_img, a, None))
                                except Exception as exc:
                                    results.append(
...
with tab_stats:
    ...
    if not records:
        ...
        # Row 1: RPN trend + share of breaches by standard
        left, right = st.columns([2, 1])
        with left:
            with ui.panel("rpn", "Worst RPN per image"):
                ui.rpn_line(
                    [f"#{i}" for i in range(1, len(records) + 1)],
                    [r.assessment.max_rpn for r in records],
                    names=[r.source for r in records],
                )
                st.caption("In screening order. Hover a point for the file "
                           "name.")
        with right:
            with ui.panel("stds", "Share of non-compliances"):
                total = stats["breach_count"] or 1
                ui.progress_rows(
                    {f"{row['short']} · {row['breaches']} of "
                     f"{stats['breach_count']}": row["breaches"] / total
                     for row in stats["by_category"]},
                    tones=["high", "info", "medium"],
                )
                st.caption(f"{stats['not_visible_count']} check(s) could not "
                           "be assessed from the images.")

        # Row 2: band and severity distributions + per-standard table
        c1, c2, c3 = st.columns([1, 1, 1.4])
        with c1:
            with ui.panel("bands", "Findings by risk band"):
                if stats["band_distribution"]:
                    ui.risk_band_bars(stats["band_distribution"], height=220)
                else:
                    st.caption("No breaches recorded.")
        with c2:
            with ui.panel("sev", "Severity distribution"):
                dist = stats["severity_distribution"]
                if dist:
                    ui.simple_bars([f"S{i}" for i in range(1, 6)],
                                   [dist.get(i, 0) for i in range(1, 6)],
                                   tone="info", height=220)
                    st.caption("S1 negligible to S5 catastrophic")
                else:
                    st.caption("No breaches recorded.")
        with c3:
            with ui.panel("bystd", "By standard"):
                st.dataframe(
                    [{
                        "Standard": row["short"],
        ...
        with ui.panel("priority", "Priority actions"):
        ...
        # Row 4: frequent breaches + open items
        f1, f2 = st.columns(2)
        with f1:
            with ui.panel("top", "Most frequent non-compliances"):
                if stats["top_checks"]:
                    st.dataframe(
                        [{
                            "Requirement": t["check"],
                    ...
                    st.caption("No breaches recorded.")
        with f2:
            with ui.panel("open", "Open items for on-site verification"):
                if stats["not_visible_by_check"]:
                    ...
                    st.caption("Every check could be assessed.")

        # Row 5: export
        with ui.panel("export", "Export"):
            ...
            with d1:
                try:
                    details = dict(
                        ...
                        inspector=inspector, position=position,
                        company=company, report_date=report_date,
                    )
                    # Only pass the fields this version of report.py accepts
                    accepted = inspect.signature(build_report).parameters
                    if not any(p.kind == p.VAR_KEYWORD
                               for p in accepted.values()):
                        details = {k: v for k, v in details.items()
                                   if k in accepted}
                    docx_bytes = build_report(records, stats, config,
                                              **details)
                    st.download_button(
...
st.divider()
st.caption(
    "AI-Assisted System for Construction Site · Work-at-Height screening · "
    "preliminary aid only · no legal or regulatory standing under the WSH Act."
)
