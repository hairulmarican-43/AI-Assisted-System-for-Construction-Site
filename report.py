...
# Risk band -> cell shading colour in the Word tables.
BAND_HEX = {
    "High": "C62828",
    "Medium": "F4A300",
    "Low": "0F9D58",
...
HEADER_HEX = "1F3864"   # dark blue used for table headers and headings
ZEBRA_HEX = "F2F2F2"    # light grey for alternate table rows

REPORT_TITLE = "AI-Assisted System for Construction Site"
REPORT_SUBTITLE = "Work-at-Height Compliance Screening Report"
...
def _cell_text(cell, text: str, *, bold: bool = False, size: int = 9,
    ...
    """
    Write text into a cell with a set size, weight and colour, replacing
    whatever was there.
    """
    cell.text = ""
...
def _add_row(table, values: list, widths: list[float],
    ...
    """
    Append a row; when a band is given, the first cell is shaded in that
    band's colour.
    """
    row = table.add_row()
...
def _footer(section, text: str) -> None:
    """Set the small grey footer text that repeats on every page."""
...
def _fmt_date(value) -> str:
    """Accept a date, datetime, or string; return DD Mon YYYY."""
...
def build_report(records: list, stats: dict, config: dict,
                 ...
                 inspector: str = "", position: str = "",
                 company: str = "", report_date=None) -> bytes:
    """
    ...
    inspector:   name of the person who ran the screening
    report_date: date of screening (date, datetime or string)
    """
    screening_date = _fmt_date(report_date) or datetime.now().strftime(
        "%d %b %Y")
    ...
    footer_bits = [b for b in (company, project, location) if b]
    _footer(
        section,
        ("  ·  ".join(footer_bits) + "\n" if footer_bits else "")
    ...
    st_run = subtitle.add_run(REPORT_SUBTITLE)
    st_run.bold = True
    st_run.font.size = Pt(14)
    st_run.font.color.rgb = RGBColor.from_string(HEADER_HEX)
    ...
    sign_widths = [2.1, 2.1, 2.3, 2.0, 1.1]
    sign = _table(doc, ["", "Name", "Position / Company", "Signature",
                        "Date"], sign_widths)

    screener_role = ", ".join(b for b in (position, company) if b)
    signers = [
        ("Screened by", inspector, screener_role, screening_date),
        ("Verified by (competent person)", "", "", ""),
        ("Approved by", "", "", ""),
    ]
    for role, name, pos_co, when in signers:
        row = sign.add_row()
        for i, width in enumerate(sign_widths):
            row.cells[i].width = Inches(width)
        _cell_text(row.cells[0], role, bold=True)
        _cell_text(row.cells[1], name)
        _cell_text(row.cells[2], pos_co)
        _cell_text(row.cells[3], "")          # always signed by hand
        _cell_text(row.cells[4], when)
